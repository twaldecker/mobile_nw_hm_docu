...
%		\begin{tabular}{ll}
%		\begin{figure}[htbp]
%		\includegraphics[width=0.50\textwidth]{img/usrp1.png}
%		\caption{GSM-Radio USRP1}
%		\label{fig:usrp1}
%		\end{figure} &
%		\begin{figure}[htbp]
%		\includegraphics[width=0.30\textwidth]{img/frequenzgen.png}
%		\caption{GSM-Radio USRP1}
%		\label{fig:fasyn}
%		\end{figure}\\
%	\end{tabular}
...
\begin{itemize}
\item \textbf{Asterisk}\\
OpenBTS benutzt eine herkömliche PBX um die Gesprächsvermittlung zu realisieren und damit das klassische Mobile Switching Center (MSC) zu ersetzen. Wir setzen dabei Asterisk ein, welches die Gespräche zwischen den MS vermittelt und zusätzliche Dienste wie beispielsweise Mailbox-Service unterstützt. 
\end{itemize}
\begin{itemize}
\item \textbf{Smqueue}\\
Zuständig für den Versand bzw. die Speicherung von SMS Nachrichten. Darüber hinaus verfügt es über "`Short Code"'-Funktionalität, die es erlaubt den Inhalt von Textnachrichten als Eingabeargumente für selbstentwickelte lokale Anwendungen zu benutzen oder an eine offizielle E-Mail-Adresse weiterzuleiten.
Smqueue hat bereits eine solche Short-Code-Anwendung namens "`register"' integriert. Hierbei handelt es sich um eine interaktive Registrierung, bei der der Benutzer eine SMS mit der gewünschten Rufnummer als Inhalt an die BTS sendet. Wird diese Nummer noch nicht verwendet, trägt "`register"' diese zusammen mit der IMSI in die Subscriber Registry ein. \footnote{Die interaktive Registration wurde von uns nicht getestet.}
\end{itemize}
\begin{itemize}
...
In Abbildung \ref{fig:openbts_system_diagram} ist nocheinmal der Kommunikationsfluß im Hinblick auf SIP aufgezeichnet. Dabei kennzeichnen die \textbf{schwarzen} Pfeile SIP-Verbindungen, die \textcolor{red}{\textbf{roten}} Pfeile Datenbankabfragen und der \textcolor{blue}{\textbf{blaue}} Pfeil ODBC-Verbindungen.
...
\begin{table}[h]
	\centering
		\begin{tabular}{ll}
			...
			& Registration, dessen Einträge von "`sipauthserve"'\\
...
Die bereits oben erwähnte Komponente "`sipauthserve"' ist dabei für die Registrierung der MS zuständig und trägt dazu  die Teilnehmer in die Datenbank \verb|sqlite3.db| ein.\\ 
...
\subsection{Subscriber Registry und Sipauthserve}
\begin{enumerate}
...
\begin{itemize}
  ...
  Mittels eines regulären Ausdrucks werden die IMSIs definiert, die sich an der BTS registrieren dürfen. Für Testzwecke ist es jedoch sinnvoll eine offene Registration zu verwenden, d.h. es darf sich jede MS verbinden. Dazu ist er Wert auf \textbf{Null} zu setzen.
...
In der Regel müssen keine weiteren Einstellungen vorgenommen werden, solang man den Standardpfad für die \verb|sqlite3.db| eingehalten hat. Hat man dies nicht, so ist der neue Dateipfad (Feld: \verb|SubscriberRegistry.db|) in der Konfigurationsdatenbank (\verb|/etc/OpenBTS/sipauthserve.db|) anzugeben. Hilfreich könnte auch das Hochsetzen des \verb|Log.Level| sein, welches standardmäßig auf \textit{WARNING} eingestellt ist und unter \textit{DEBUG} deutlich mehr Hinweise bzgl. der Registry-Einträge offenbart.

\subsection{Smqueue}
Auch an der Konfiguration von smqueue muss zwangsläufig keine Änderung vorgenommen werden. Allerdings enthält die Konfigurationsdatenbank (\verb|/etc/OpenBTS/smqueue.db|) weitaus mehr Einträge als die von sipauthserve. Gut die Hälfte dieser Parameter beziehen sich auf die "`Short Code"'-Funktionalität.

\subsection{Asterisk}
Die Konfiguration von Asterisk beschränkt sich in diesem Dokument auf die Intra-BTS-Kommunikation, d.h. es können nur Gespräche zwischen MS und MS bzw. MS und Asterisk-Diensten (Echo-Test, VoiceMail) statt finden, nicht jedoch von oder nach Ausserhalb (Festnetz, andere SIP-Teilnehmer) telefoniert werden.
\subsubsection{Teilnehmereintrag}
\label{asterisk_teilnehmer}
Damit die MS untereinander telefonieren können, müssen die Asterisk-Konfigurationsdateien \verb|sip.conf| und \verb|extensions.conf| im Verzeichnis \verb|/etc/asterisk/| angepasst werden. Als Beispiel wird die \textbf{IMSI 001010000000000} verwendet und dieser die Teilnehmerrufnummer \textbf{2101} zugeordnet.\\
...
type=friend           ; MS ruf uns bzw. wir rufen MS an
...
In \verb|extensions.conf| muss je IMSI ein Eintrag in der ihr zugeordneten Extension hinzugefügt werden:
\begin{figure}[ht]
\setbox0\vbox{\small
\begin{verbatim}
...
\subsubsection{Echo-Test}
Zu Testzwecken empfielt es sich einen sog. "`Echo-Test"' unter Asterisk einzustellen. Dabei wird alles was man sagt als Echo zurückgeschickt. Zum einen erkennt man dadurch welche Latenzzeit zwischen MS und Asterisk besteht und zum anderen lässt sich ein Sprachkanal ohne die Notwendigkeit einer zweiten MS einfach aufbauen.
Um den Echo-Test zu aktivieren werden einfach folgende Zeilen in die \verb|extensions.conf| innerhalb des Kontext \verb|[sip-external]| eingefügt:
\newpage
\begin{figure}[ht]
\setbox0\vbox{\small
\begin{verbatim}
...
In der Praxis zeigte sich, dass die Sprachverzögerung um die 0,5 Sekunden liegt, was mit hoher Wahrscheinlichkeit an OpenBTS liegt. Ein zweiter Test zwischen Asterisk und eines herkömmlichen Software SIP-Clients (direkte VoIP-Verbindung) wies nämlich keine nennenswerte Latenz auf.
...
Das Regelwerk des Benutzers mit der Teilnehmerrufnummer 2101 besagt folgendes: Zuerst wird versucht den Benutzer \textbf{IMSI001010000000000} über SIP zu erreichen (siehe Kapitel \ref{gsmsip}). Falls er im Netz registriert ist und die MS erfolgreich aufgefunden wird, wird man nun maximal 20 Sekunden lang anläuten und nach Ablauf dieser Zeit zu Regel 2 springen. Ist der Benutzer erst gar nicht im Netz registriert, so wird unmittelbar zu Regel 2 gesprungen. Regel 2 definiert den eigentlichen Mailbox-Befehl. Der Anrufende hört die Mailboxansage und hat die Möglichkeit eine Nachricht zu hinterlassen. Hat er dies getan, so kann er entweder direkt auflegen oder mittels der Rautetaste die Aufnahme beenden. Regel 3 spielt daraufhin ein "`Auf wiedersehen"'-Soundfile ab und Regel 4 beendet letztendlich den Anruf.
...
In der Praxis kam es zu Problemen bei der interaktiven Mailbox-Abfrage. Die Kennung und/oder das Passwort wurden als flasch von Asterisk betrachtet und eine erfolgreiche Authentifizierung war nicht möglich. Der Grund hierfür dürfte an einem nicht unterstützten bzw. nicht konfigurierten DTMF-Modus liegen, mittels dem die Tasteneingabe (Tastentöne) übertragen werden. Aus Zeitgründen und weil die Mailbox-Abfrage für unser Projektziel nicht relevant war, wurde der Sache nicht weiter auf den Grund gegangen.
\end{enumerate}

\section{Benutzung von OpenBTS}
\subsection{Start der Dienste}
...
\subsection{Registrierung einer MS an OpenBTS}
Mit Hilfe der bereitgestellten Mobiletelefone vom Typ "`Nokia 3330"' und der programmierbaren SIM-Karten von Giesecke \& Devrient konnte nun eine Registrierung in unserem GSM Testnetz namens "`OpenBTS HM"' stattfinden. Dank der offenen Registrierung (\verb|Control.LUR.OpenRegistration|) können wir uns einfach am Netz registrieren und erhalten zudem eine SMS, dessen Inhalt wir mit dem Konfigurationsparameter \verb|Control.LUR.OpenRegistration.Message| bestimmen können. Wichtiger Bestandteil dieser Kurzmitteilung ist die IMSI der SIM-Karte mit der sich die MS an der BTS registriert hat. Nun kann ein Teilnehmereintrag in Asterisk, wie in Kapitel  \ref{asterisk_teilnehmer} beschrieben, erfolgen. Folgendes Beispiel zeigt den SMS-Inhalt für die IMSI 262071111111111:
\begin{figure}[ht]
\setbox0\vbox{\small
\begin{verbatim}
...
Konnte OpenBTS erfolgreich gestartet werden, hat man nun das Command Line Interface (CLI) vor sich:
\begin{verbatim}
OpenBTS>
\end{verbatim}
...
\begin{itemize}
\item Befehl: \verb|calls|\\
Listet derzeitig aktive Gesprächs- bzw. SMS-Aktivitäten auf.
\begin{figure}[ht]
\setbox0\vbox{\small
\begin{verbatim}
OpenBTS> calls
...
\end{figure}\\
Dabei ist \verb|2060207953| die Transaktions-ID, \verb|C0T1| die C0-ARFCN und der dabei verwendete Zeitschlitz, \verb|TCH/F| die Kanalart und \verb|IMSI=001010000000000| die IMSI des Teilnehmers. Die restlichen Angaben beziehen sich auf die SIP-Verbindung (ID, SIP-Status und Verbindungsdauer).
\end{itemize}
\begin{itemize}
\item Befehl: \verb|chans|\\
...
\end{figure}\\
Im obigen Beispiel handelt es sich um einen Verkehrskanal im Full-Rate Modus (\verb|TCH/F|) der den ersten Zeitschlitz (\verb|TN=1|) belegt. \verb|TXPWR| ist die Sendeleistung der MS und \verb|TXTA| der Timing Advance Wert.
\end{itemize}
\begin{itemize}
\item Befehl: \verb|tmsis|\\
...
\end{figure}\\
Im Beispiel wurde der IMSI \verb|001010000000000| die TMSI \verb|1| zugeordnet. Dieser Eintrag besteht seit 138s (\verb|age|) und genau so lange ist die MS im Netz registriert (\verb|used|). 
\end{itemize}
